...
function selectLoad() {
    ...
    $.ajax({
        ...
        success: function (data) {
            let setData = data.map(item => ({value: item.type, name: item.description}))
            utils.loadTypes(["dict_type"], ["type"], [{width: "150px", noneSelectedText: '类别', liveSearch: true, multiple: true, setData: setData}]);
            ...
            columns: [
                ...
                {visible: false, field: 'remarks', title: '备注信息'},
                {
                    title: '状态', field: 'delFlag', formatter: (value, index) => {
                        return value === 0 ? '<span class="label label-danger">禁用</span>' : '<span class="label label-primary">正常</span>';
                    }
                },
                {
                    title: '操作', field: 'id', align: 'center', formatter: (value, row, index) => {
                        return utils.renderButtons([
                            {html: `<a class="btn btn-primary btn-sm" href="#" title="编辑" onclick="edit('${row.id}')"><i class="fa fa-edit"></i></a> `, perm: 'common:sysDict:edit'},
                            {html: `<a class="btn btn-warning btn-sm" href="#" title="删除" onclick="remove('${row.id}')"><i class="fa fa-remove"></i></a> `, perm: 'common:sysDict:remove'},
                            {html: `<a class="btn btn-success btn-sm" href="#" title="增加下級" onclick="addD('${row.type}', '${row.description}')"><i class="fa fa-plus"></i></a> `, perm: 'common:sysDict:add'}
                        ], row);
                    }
                }
            ]
